CREATE OR REPLACE TRIGGER MEME_STATION
	BEFORE INSERT ON TacheRoutine
	FOR EACH ROW
DECLARE
	rangMax int;
BEGIN
	SELECT Min(rang) INTO rangMax
  	FROM TacheRoutine
  	WHERE ;
	
	SELECT COUNT (numVehicule) FROM TacheRoutine
	WHERE numVehicule=:new.numVehicule AND rang=rangMax
END;
/
